...
- spec/navigation.md — registry of this project’s concrete `spec/extend/*` and optional extra `spec/design/*.md` (besides `hla.md`); see Folder Structure here for all other paths (required).
- spec/design/hla.md — project high-level architecture (required).
- spec/design/{name}.md — other architecture documents (optional; ADRs, notes, etc.); each file besides `hla.md` must be listed in `spec/navigation.md`.
...
No other files are permitted in `spec/`: paths must match this Folder Structure, and any concrete `spec/extend/*.md` or optional extra `spec/design/*.md` (other than `hla.md`) must be listed in `spec/navigation.md`. Do not create READMEs or extra docs.
...
## Step 2: Self Spec Review (AI subagent)
...
→ prompt: "Spec ready. Reply 'spec review passed' to proceed."

---

## Step 3: Spec Review (User)
...
## Step 5: Self Code Review (AI subagent)
